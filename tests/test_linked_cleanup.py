...
class RemoveNodeByIdTests(unittest.TestCase):
    def setUp(self):
        install_anki_stubs()
        _clear_package_modules()
        self.lc = import_plugin_module("manager.linked_cleanup")
    ...
    def test_non_dict_input(self):
        self.assertFalse(self.lc.remove_node_by_id(None, "x"))
        self.assertFalse(self.lc.remove_node_by_id("string", "x"))
        self.assertFalse(self.lc.remove_node_by_id(123, "x"))
        self.assertFalse(self.lc.remove_node_by_id([1, 2], "x"))

    def test_match_at_root_without_parent_does_not_pop(self):
        # When the match is at the root and no parent context is given,
        # the function should NOT pop and should return False (continues).
        root = {"id": "target", "children": [{"id": "child"}]}
        result = self.lc.remove_node_by_id(root, "target")
        self.assertFalse(result)
        # Root still exists with its children intact
        self.assertEqual(root["id"], "target")
        self.assertEqual(len(root["children"]), 1)

    def test_match_with_parent_context_removes_and_returns_true(self):
        siblings = [{"id": "a"}, {"id": "b"}, {"id": "c"}]
        root = {"id": "root", "children": siblings}
        result = self.lc.remove_node_by_id(root["children"][1], "b", siblings, 1)
        self.assertTrue(result)
        self.assertEqual([n["id"] for n in siblings], ["a", "c"])

    def test_recursive_removal_in_grandchild(self):
        root = {
            "id": "root",
            "children": [
                {"id": "a", "children": [{"id": "deep"}, {"id": "other"}]},
            ],
        }
        result = self.lc.remove_node_by_id(root, "deep")
        self.assertTrue(result)
        self.assertEqual(
            [c["id"] for c in root["children"][0]["children"]],
            ["other"],
        )

    def test_only_first_match_removed(self):
        # If duplicate ids exist (unusual but possible), only the first found is removed.
        root = {
            "id": "root",
            "children": [
                {"id": "dup"},
        ...
        result = self.lc.remove_node_by_id(root, "dup")
        self.assertTrue(result)
        self.assertEqual(len(root["children"]), 1)

    def test_empty_children_list(self):
        root = {"id": "root", "children": []}
        self.assertFalse(self.lc.remove_node_by_id(root, "anything"))

    def test_returns_false_when_id_missing(self):
        root = {"id": "root", "children": [{"id": "a", "children": [{"id": "b"}]}]}
        self.assertFalse(self.lc.remove_node_by_id(root, "ghost"))
